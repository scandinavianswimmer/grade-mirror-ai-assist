...
import { buildTeacherFeedbackExport, isScoreWithheld, teacherFeedbackFilename } from '@/lib/teacherFeedbackExport';
...
const SubmissionDetail = () => {
  ...
  const [reviewTab, setReviewTab] = useState<'rubric' | 'feedback'>('rubric');
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setReviewTab('rubric'); }, [id]);
  ...
  const visibleAnnotations = [...placed, ...unplaced];
  ...
  const reviewedSubmissionCount = submissionStack.filter((item) => isApprovedStatus(item.status)).length;
  const closerLookSubmissionCount = submissionStack.filter((item) => (
    item.status === 'needs_review' || item.status === 'grade_error'
  )).length;

  const showAnnotation = (annotationId: string) => {
    setActive(annotationId);
    setReviewTab('feedback');
  };
    ...
    placed.forEach((a) => {
      ...
      nodes.push(
        <mark
          ...
          onClick={() => showAnnotation(a.id)}
          onKeyDown={(ev) => { if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); showAnnotation(a.id); } }}
  ...
  const scoreWithheld = isScoreWithheld(grade?.flags);
      ...
      <main id="main-content" tabIndex={-1} className="container mx-auto px-4 py-8">
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                ...
                {submissionStack.length > 0 && <span>{reviewedSubmissionCount} reviewed</span>}
                {closerLookSubmissionCount > 0 && <span>{closerLookSubmissionCount} {closerLookSubmissionCount === 1 ? 'needs' : 'need'} a closer look</span>}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center" role="group" aria-label="Move through submission stack">
                {previousSubmission ? (
                  <Button asChild variant="outline" size="icon" className="h-11 w-11 rounded-r-none">
                    <Link to={`/submission/${previousSubmission.id}`} aria-label={`Previous submission: ${previousSubmission.studentName || 'student'}`}>
                      <ChevronLeft aria-hidden="true" className="h-4 w-4" />
                    </Link>
                  </Button>
                ) : <Button variant="outline" size="icon" className="h-11 w-11 rounded-r-none" disabled aria-label="No previous submission"><ChevronLeft aria-hidden="true" className="h-4 w-4" /></Button>}
                {nextSubmission ? (
                  <Button asChild variant="outline" size="icon" className="h-11 w-11 rounded-l-none border-l-0">
                    <Link to={`/submission/${nextSubmission.id}`} aria-label={`Next submission: ${nextSubmission.studentName || 'student'}`}>
                      <ChevronRight aria-hidden="true" className="h-4 w-4" />
                    </Link>
                  </Button>
                ) : <Button variant="outline" size="icon" className="h-11 w-11 rounded-l-none border-l-0" disabled aria-label="No next submission"><ChevronRight aria-hidden="true" className="h-4 w-4" /></Button>}
              </div>
              {grade && !approved && (
        ...
        <div className="mx-auto grid max-w-[1500px] gap-6 lg:grid-cols-[minmax(0,1fr)_390px]">
          ...
          <aside aria-label="Rubric and feedback" className="space-y-4">
            ...
            {pendingFinalize && (
              <section className="rounded-lg border border-primary/40 bg-primary/5 p-4" aria-labelledby="approval-review-heading">
                <h2 id="approval-review-heading" className="text-sm font-semibold">Before you approve</h2>
            ...
            {grade && (
              <Card className="overflow-hidden lg:sticky lg:top-24 lg:z-10">
                <Tabs
                  value={reviewTab}
                  onValueChange={(value) => {
                    if (value === 'rubric' || value === 'feedback') setReviewTab(value);
                  }}
                >
                  <TabsList className="grid h-12 w-full grid-cols-2 rounded-none border-b border-border bg-transparent p-0" aria-label="Review details">
                    <TabsTrigger value="rubric" className="h-12 rounded-none border-b-2 border-transparent text-sm data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:shadow-none">Rubric</TabsTrigger>
                    <TabsTrigger value="feedback" className="h-12 rounded-none border-b-2 border-transparent text-sm data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:shadow-none">Feedback</TabsTrigger>
                  </TabsList>

                  <TabsContent value="rubric" className="m-0 space-y-4 p-4 lg:max-h-[calc(100vh-9rem)] lg:overflow-y-auto">
                    ...
                    {Array.isArray(grade.criteria) && grade.criteria.length > 0 && (
                      <div className="divide-y divide-border/70 border-y border-border/70">
                        <div className="flex items-center justify-between py-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                          <span>Criteria</span>
                          <span>Score</span>
                        </div>
                        {grade.criteria.map((criterion, index) => (
                          <div key={index} className="py-3 text-sm">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <span className="shrink-0 rounded-md border border-border/80 bg-background px-2 py-1 text-xs font-medium text-foreground">{scoreWithheld ? 'Not scored' : `${criterion.score}/${criterion.maxScore}`}</span>
                            </div>
                    ...
                    <div>
                      <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{scoreWithheld ? 'Score withheld' : 'Total'}</p>
                      {scoreWithheld ? (
                        <div className="mt-1">
                          <p className="text-2xl font-semibold text-suggestion">No score proposed</p>
                          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">This submission is waiting for teacher review before a score is assigned.</p>
                        </div>
                      ) : (
                        <div className="mt-1 flex items-baseline gap-2">
                          <span className="text-3xl font-semibold text-primary">{grade.overall_score ?? '—'}</span>
                          <span className="text-base text-muted-foreground">/ {grade.overall_max ?? 100}</span>
                    ...
                    {grade.summary_feedback && (
                      <div>
                        <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Draft summary</p>
                  ...
                  <TabsContent value="feedback" className="m-0 space-y-3 p-4 lg:max-h-[calc(100vh-9rem)] lg:overflow-y-auto">
                    <div className="flex flex-wrap items-start justify-between gap-2 border-b border-border/70 pb-3">
                      <div>
                        <p className="text-sm font-semibold">Margin notes ({visibleAnnotations.length})</p>
                      ...
                      {visibleAnnotations.length > 0 && (
                        <div className="flex flex-wrap items-center gap-1">
                          <Button size="sm" variant="ghost" className="min-h-11 gap-1 px-2 text-xs" onClick={() => bulkSetStatus('accepted')}><Check aria-hidden="true" className="h-3.5 w-3.5" /> Accept all</Button>
                          <Button size="sm" variant="ghost" className="min-h-11 gap-1 px-2 text-xs text-muted-foreground" onClick={() => bulkSetStatus('rejected')}><X aria-hidden="true" className="h-3.5 w-3.5" /> Dismiss all</Button>
                        </div>
                      )}
                    </div>

                    {visibleAnnotations.length === 0 ? (
                      <p className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">No margin notes were drafted for this paper.</p>
                      ...
                      <div key={annotation.id} className={`rounded-md border p-3 text-sm ${active === annotation.id ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}>
                        <div className="mb-2 flex flex-wrap items-center gap-2">
                          ...
                          {annotation.status === 'accepted' && <span className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-praise"><ShieldCheck aria-hidden="true" className="h-3.5 w-3.5" /> Accepted</span>}
                          {annotation.status === 'edited' && <span className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-suggestion"><Pencil aria-hidden="true" className="h-3.5 w-3.5" /> Edited</span>}
                        </div>
                        ...
                        {editing === annotation.id ? (
                          ...
                          <>
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-1">
                              <Button size="sm" variant={annotation.status === 'accepted' ? 'default' : 'outline'} className="min-h-11 gap-1 px-3 text-xs" onClick={() => setStatus(annotation, 'accepted')}><Check aria-hidden="true" className="h-3.5 w-3.5" /> Accept</Button>
                              <Button size="sm" variant="ghost" className="min-h-11 gap-1 px-3 text-xs" onClick={() => { setEditing(annotation.id); setDraft(annotation.comment); }}><Pencil aria-hidden="true" className="h-3.5 w-3.5" /> Edit</Button>
                              <Button size="sm" variant="ghost" className="min-h-11 gap-1 px-3 text-xs text-muted-foreground" onClick={() => setStatus(annotation, 'rejected')}><X aria-hidden="true" className="h-3.5 w-3.5" /> Dismiss</Button>
                            </div>
                          </>
                        )}
                      </div>
            ...
            {!grade && essay && (
              <Card className="border-dashed p-6 text-center text-sm text-muted-foreground">
                ...
                No draft feedback yet. Ask Mr Selby for a first pass, then review the proposed rubric score and each margin note.
              </Card>
            )}
          </aside>
        </div>
