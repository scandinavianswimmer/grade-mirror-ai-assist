...
const sections: LegalPageSection[] = [
  ...
  { id: 'data', title: 'Data aiTA handles' },
...
const Privacy = () => (
  <LegalPreviewLayout
    ...
    summary="aiTA works with sensitive classroom material. This launch preview explains what the product currently handles, why it is needed, and which controls teachers have—without pretending the unfinished legal details are settled."
    sections={sections}
  >
    <LegalSection id="scope" title="Scope and status">
      <p>
        This overview covers aiTA&apos;s teacher-facing grading workspace and the data processed when an authorized user creates an account, prepares an assignment, uploads student work, reviews feedback, or manages a plan.
      </p>
      <p>
    ...
    <LegalSection id="data" title="Data aiTA handles">
      <p>Depending on how a teacher uses the product, aiTA may store or process:</p>
      <ul className="list-disc space-y-2 pl-6 marker:text-primary">
        ...
        <li><strong className="text-foreground">Billing metadata</strong> needed to manage a paid plan. Payment-card details are handled by the configured payment provider rather than entered into aiTA&apos;s application database.</li>
    ...
    <LegalSection id="uses" title="How data is used">
      <p>aiTA uses the information above to provide and operate the grading workflow. That includes:</p>
      <ul className="list-disc space-y-2 pl-6 marker:text-primary">
      ...
      <p>
        aiTA is not designed to sell classroom content or use it for advertising. Final counsel review will convert that launch position into binding policy language.
      </p>
    </LegalSection>

    <LegalSection id="ai-providers" title="AI and service providers">
      <p>
        When a teacher asks aiTA to analyze or grade work, relevant assignment and submission content may be sent to the configured AI provider so it can produce a draft. The current product architecture uses Google Gemini for grading and feedback generation.
      </p>
      <p>
      ...
      <p>
        Teacher edits or student content can become a saved personalization example only through the product&apos;s optional consent flow. Turning on personalization is separate from using aiTA&apos;s ordinary grading workflow.
      </p>
    </LegalSection>
    ...
    <LegalSection id="retention" title="Retention and deletion">
      <p>
        aiTA includes teacher-selectable retention settings for uploaded files and grading data. It also includes controls to remove unfinalized AI-generated content and to delete user-owned workspace data.
      </p>
      <p>
    ...
    <LegalSection id="schools" title="Student and school data">
      <p>
        Student work may contain personal information even when a student does not have an aiTA account. Teachers and institutions should use aiTA only when they have authority to provide that material and should follow their school or district&apos;s review, notice, consent, and records requirements.
      </p>
      <p>
